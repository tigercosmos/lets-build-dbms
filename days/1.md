...
The program might need to have configuration and arguments.

If the user doesn't give arguments to run the program, the program should adopt the default value from configuration.

There are some setting would be covered in configuration and arguments. So far I could think of is that, the setting includes the port of the server and wether the server runs in daemon mode. There might be more, and will be added later.

For doing that, you can see there is a `.env` file now, which is for configuration. The `dotenv_codegen` crate helps us to parse the value from `.env`. There is also a `cli.yml` file, which is the schema of arguments of the program, powered by `clap` crate.

You could run the following command to see how to set arguments for StellarSQL once you `cargo build` an executable file. Assume it is a debug build, the file is at `target/debug/stellar_sql`.
...
So now we add some code in the main program.
